      ...
      <div class="flex items-center gap-1">
		...
		<div>
			<h2 class="text-subtitle font-semibold">
				{{ $t("workbook.workbook.information") }}
			</h2>
          ...
          "
				>
					...
					<img
						v-if="workBook.image"
						:class="`h-full ${localImage ? 'hidden' : 'visible'}`"
						:src="workBook.image"
						alt="cover book"
					/>
          ...
          "
				>
					<div class="flex justify-center items-center text-xl gap-2">
						<label class="w-32" for="title"
							>{{ $t("workbook.workbook.title") }}:
						</label>
						<input
							type="text"
              ...
              "
					...
					<div class="flex justify-center items-center text-xl gap-2">
						<label class="w-32" for="author"
							>{{ $t("workbook.workbook.published") }}:</label
						>
						<label class="w-full text-left" for="author">{{
							workBook.status
						}}</label>
					</div>
					<div class="flex justify-center items-center text-xl gap-2">
						<label class="w-32" for="author"
							>{{ $t("workbook.workbook.price") }}:</label
						>
						<div class="flex gap-2 w-full">
							<input
								type="number"
            ...
            border border-gray-500
          "
					:placeholder="$t('workbook.workbook.describePlaceHolder')"
			...
			async goToEditor() {
				if (this.idWorkBook === "new") {
					...
					this.$router.push({
						...
						params: { idWorkBook: newWorkbook.id },
					});
				} else {
					...
					this.$router.push({
						name: "workbook-rich-text",
						params: { idWorkBook: this.idWorkBook },
					});
				}
			},
			loadWorkBook() {
			...
			deleteCurrentWorkbook() {
				Swal.fire({
					...
					confirmButtonText: this.$t(
						"swallAlertGeneral.confirmDelete.confirmButtonText",
					),
					denyButtonText: this.$t(
						"swallAlertGeneral.confirmDelete.denyButtonText",
					),
				}).then(async (result) => {
